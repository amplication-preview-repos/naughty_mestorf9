/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
  IsNumber,
  Min,
  Max,
} from "class-validator";
import { OrderUpdateManyWithoutHaulersInput } from "./OrderUpdateManyWithoutHaulersInput";
import { Type } from "class-transformer";
import { QuoteUpdateManyWithoutHaulersInput } from "./QuoteUpdateManyWithoutHaulersInput";

@InputType()
class HaulerUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => OrderUpdateManyWithoutHaulersInput,
  })
  @ValidateNested()
  @Type(() => OrderUpdateManyWithoutHaulersInput)
  @IsOptional()
  @Field(() => OrderUpdateManyWithoutHaulersInput, {
    nullable: true,
  })
  orders?: OrderUpdateManyWithoutHaulersInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  pricing?: number | null;

  @ApiProperty({
    required: false,
    type: () => QuoteUpdateManyWithoutHaulersInput,
  })
  @ValidateNested()
  @Type(() => QuoteUpdateManyWithoutHaulersInput)
  @IsOptional()
  @Field(() => QuoteUpdateManyWithoutHaulersInput, {
    nullable: true,
  })
  quotes?: QuoteUpdateManyWithoutHaulersInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  zipCode?: string | null;
}

export { HaulerUpdateInput as HaulerUpdateInput };
