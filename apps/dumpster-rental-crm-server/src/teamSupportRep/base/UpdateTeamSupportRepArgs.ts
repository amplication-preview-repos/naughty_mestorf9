/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TeamSupportRepWhereUniqueInput } from "./TeamSupportRepWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { TeamSupportRepUpdateInput } from "./TeamSupportRepUpdateInput";

@ArgsType()
class UpdateTeamSupportRepArgs {
  @ApiProperty({
    required: true,
    type: () => TeamSupportRepWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => TeamSupportRepWhereUniqueInput)
  @Field(() => TeamSupportRepWhereUniqueInput, { nullable: false })
  where!: TeamSupportRepWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => TeamSupportRepUpdateInput,
  })
  @ValidateNested()
  @Type(() => TeamSupportRepUpdateInput)
  @Field(() => TeamSupportRepUpdateInput, { nullable: false })
  data!: TeamSupportRepUpdateInput;
}

export { UpdateTeamSupportRepArgs as UpdateTeamSupportRepArgs };
